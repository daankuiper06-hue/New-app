...
export default async function InvoicesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Facturen"
        description="Alle materiaalfacturen met nette totalen en vaste navigatie."
        action={
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-slate-200 text-slate-500">
              <tr>
                ...
                <th className="py-3 pr-4">Regels</th>
            ...
            <tbody>
              {invoices.map((invoice) => (
                <tr key={invoice.id} className="border-b border-slate-100 last:border-0">
                  <td className="py-3 pr-4">{toInputDate(invoice.invoiceDate)}</td>
                  ...
                  <td className="py-3 pr-4">{invoice.items.length}</td>
